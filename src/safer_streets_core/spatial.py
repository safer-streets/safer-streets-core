...
def get_census_boundaries(
    ...
    *,
    resolution: Resolution = "GC",
    overlapping: gpd.GeoDataFrame | None = None,
) -> gpd.GeoDataFrame:
...
def get_street_network(boundary: gpd.GeoDataFrame, *, network_type: str = "drive", **args: Any) -> gpd.GeoDataFrame:
    ...
    G = ox.graph_from_polygon(
        boundary.to_crs(epsg=4326).iloc[0].geometry,
...
def map_to_spatial_unit(
    raw_crime_data: gpd.GeoDataFrame,
    boundary: gpd.GeoDataFrame,
    area_type: SpatialUnit,
    **kwargs,
...
def hex_neighbours(spatial_unit: int):
    """This returns neighbour hex ids for the E&W hex grid generated by scripts/ew_hex200.py"""
    sign = -1 if spatial_unit > _INTERLEAVE else 1
    return [
        spatial_unit - 1,
        spatial_unit + 1,
        spatial_unit + sign * _INTERLEAVE,
        spatial_unit + sign * (_INTERLEAVE + 1),
